...
def train_step(batch_data, run_info):
    ...
    imgs = imgs.to(_device()).type(torch.float32)  # to NCHW
    imgs = imgs.permute(0, 3, 1, 2).contiguous()

    # HWC
    true_np = true_np.to(_device()).type(torch.int64)
    true_hv = true_hv.to(_device()).type(torch.float32)
    ...
    if model.module.nr_types is not None:
        ...
        true_tp = torch.squeeze(true_tp).to(_device()).type(torch.int64)
...
def valid_step(batch_data, run_info):
    ...
    imgs_gpu = imgs.to(_device()).type(torch.float32)  # to NCHW
...
def infer_step(batch_data, model):
    ...
    patch_imgs_gpu = patch_imgs.to(_device()).type(torch.float32)  # to NCHW
