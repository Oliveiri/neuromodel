...
import numpy as np
# === Windows 下手动注入 OpenSlide DLL 路径，必须放在 import openslide 之前 ===
OPENSLIDE_BIN = r"C:\Users\29698\Desktop\model-5.6\5.18 resnet+clam\ResNet分类\open_slide\openslide-bin-4.0.0.3-windows-x64\bin"

if os.name == "nt":
    if not os.path.isdir(OPENSLIDE_BIN):
        raise FileNotFoundError(f"OpenSlide bin 目录不存在: {OPENSLIDE_BIN}")

    # 先把 bin 加进 PATH，给 ctypes/子依赖兜底
